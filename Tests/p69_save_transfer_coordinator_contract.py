...
for marker in [
    '@interface ZONSaveTransferCoordinator',
...
for marker in [
    ...
    'presentRemoteDownloadFromViewController:host',
    'presentCloudSaveFromViewController:host',
    '[self checkCloudSaveStatus];',
]:
    assert marker in pubg, f'missing PubgLoad compatibility shim marker: {marker}'

for forbidden in [
    'ZONRemoteDownloadService',
    'ZONRestoreAPI',
    'ZONCloudSaveService',
    'getKeychain',
    'SVProgressHUD',
...
assert version == 'v1_p69', f'unexpected VERSION: {version}'
