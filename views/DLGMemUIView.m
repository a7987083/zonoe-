...
- (void)initConsoleButton {
    ...
    [button setTitle:@"ZONOE" forState:UIControlStateNormal];
    [button.titleLabel setFont:[UIFont fontWithName:@"AmericanTypewriter-Bold" size:12]];
    [button setTitleColor:[UIColor redColor] forState:UIControlStateNormal];//图标颜色
...
- (void)initSearchViewContainer {
    ...
    NSArray *cv = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-45-[v(32)]" options:0 metrics:nil views:views];
...
- (void)initSearchValueInput {
    ...
    NSArray *ch = [NSLayoutConstraint constraintsWithVisualFormat:@"H:[lbl]-2-[input]-2-[lianhe]-3-[quangai]-30-[btn]" options:0 metrics:nil views:views];
    [self.vSearch addConstraints:ch];
...
- (void)initOptionViewContainer {
    ...
    NSArray *ch = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-8-[v]-8-|" options:0 metrics:nil views:views];
    [self addConstraints:ch];
    NSArray *cv = [NSLayoutConstraint constraintsWithVisualFormat:@"V:[vv]-15-[v]" options:0 metrics:nil views:views];
    [self.vContent addConstraints:cv];
...
- (void)initResultLabel {
    ...
    lbl.text = @"双指点击三次隐藏";
    lbl.text = @"如果不会修改,可点击屏幕中间底部 查看教程";
...
- (void)initMemoryViewContainer {
    ...
    NSArray *cv = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-15-[v(32)]" options:0 metrics:nil views:views];
...
- (void)initMemoryTextView {
    ...
    tv.translatesAutoresizingMaskIntoConstraints = NO;
    tv.font = [UIFont fontWithName:@"Courier New" size:16];
    tv.backgroundColor = [UIColor clearColor];
    ...
    tv.text =@
    "😉高级教程：联合搜索\n\n比如:搜索F32 8888联合1，1改9999，\n修改器选择F32 输入框输入8888，按(精确搜索)按钮，等待出结果后 \n输入框输入1 按(联合)按钮 等出结果 此时列表里有8888和1的内存 \n然后 输入框输入1 按(精确搜索)按钮就可以筛选出1， 输入框输入9999按(全改)即可\n\n联合搜索可以联合N个 不限次数 一般联合1~3个即可找到结果\n联合搜索适用于较小的数值比如1 搜索结果可能上亿个 所以只能先搜索它附近较大的值 再联合找到它使用\n\n😂初级教程：精确搜索(最常规最简单的修改)\n 一般游戏都是I32数据  \n 比如我现在的金币是688个\n 1⃣️第一步输入框输入688金币数选择I32格式格式，按按(精确搜索)按钮 在点击键盘的搜索按钮 \n出结果后可能会有几百甚至上千,此时不要着急 \n\n2⃣️第二部此时游戏里花掉金币或获取金币让值改变\n比如花掉8金币 变成680\n\n3⃣️第三步输入框输入680金币数 再点击键盘的搜索按钮,此时搜索的结果变少.当结果少于10个以内后,可以尝试全改.或者手动修改到你想要的数字比如21亿, \n\n修改器右下角的--刷新--这个功能非常好用\n\n ⚠️为了避免修改导致游戏闪退,一定要确保搜索的结果少于10个,这样修改起来才稳妥.  \n如果你按照上面的教程尝试修改后结果还是很多. 就请重复第二步和第三步即可\n\n如果还不懂,最好去百度搜索DLG修改教程,学一学.";
