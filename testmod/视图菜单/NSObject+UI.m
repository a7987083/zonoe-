...
- (void)vip菜单显示
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [topVC presentViewController:menu
                            animated:NO
                          completion:^{
            // Diagnostic B: open zonoe only. Do not install callback hooks.
            NSURL *requestURL = ZONUDIDBridgeRequestURL();
            if (requestURL) {
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(350 * NSEC_PER_MSEC)),
                               dispatch_get_main_queue(), ^{
                    NSLog(@"[zonoemenu][DIAG][udid] open-only begin");
                    [UIApplication.sharedApplication openURL:requestURL
                                                     options:@{}
                                           completionHandler:^(BOOL success) {
                        NSLog(@"[zonoemenu][DIAG][udid] open-only result=%d", success);
                    }];
                });
            }
        }];
