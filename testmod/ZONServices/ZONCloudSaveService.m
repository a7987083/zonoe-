...
+ (instancetype)sharedService
{
    ...
    dispatch_once(&onceToken, ^{ service = [[ZONCloudSaveService alloc] init]; });
    return service;
}
                               ...
                               completion:(ZONCloudSaveMetadataCompletion)completion
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@.json", metadataBaseURLString, bundleIdentifier]];
    if (!url) {
...
- (NSString *)effectiveDownloadAddressForFunction:(NSDictionary *)functionDictionary
{
    NSString *buttonName = [functionDictionary[@"按钮名字"] isKindOfClass:[NSString class]] ? functionDictionary[@"按钮名字"] : @"";
    if ([buttonName containsString:@"数据"] || [buttonName containsString:@"解说"] || [buttonName containsString:@"存档"]) return nil;
    id address = functionDictionary[@"下载地址"];
    return [address isKindOfClass:[NSString class]] ? address : @"";
}
                                   ...
                                   completion:(ZONCloudSaveDownloadResolutionCompletion)completion
{
    ...
    NSString *escapedDevice = [deviceIdentifier stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    NSString *entitlementString = [NSString stringWithFormat:@"%@%@", entitlementBaseURLString, escapedDevice ?: @""];
    NSURL *entitlementURL = [NSURL URLWithString:entitlementString];
    if (!entitlementURL) {
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:entitlementURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (error) {
