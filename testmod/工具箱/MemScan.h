...
class JJMemoryEngine
{
    ...
    {
        vm_size_t size = 0;
        kern_return_t error = vm_read_overwrite(this->task, (vm_address_t)addr, len, (vm_address_t)buf, &size);
    ...
    uint64_t ScanData(uint64_t buffer, uint64_t size, void* target, int type)
    {
        ...
        if(type==JJ_Search_Type_Float) {
            ...
            float d = this->float_tolerance;
            float value_up =  t + d;
            float value_down = t - d;
            while(p<=end) {
                float v = *(float*)p;
            ...
            double d = this->float_tolerance;
            double value_up =  t + d;
            double value_down = t - d;
            while(p<=end) {
                double v = *(double*)p;
                if(v>=value_down && v<=value_up) break;
                p+=len;
            ...
            while(p<=end && *(uint8_t*)p != value) p+=len;
        } else if(len==2) {
            uint16_t value = *(uint16_t*)target;
            while(p<=end && *(uint16_t*)p != value) p+=len;
        } else if(len==4) {
            uint32_t t = *(uint32_t*)target;
            uint32_t d = this->int_Max;
            uint32_t value_up =  t + d;
            uint32_t value_down = t - 0;
            while(p<=end) {
            ...
            uint64_t d = this->int_Max;
            uint64_t value_up =  t + d;
            uint64_t value_down = t - 0;
            while(p<=end) {
...
public:
    ...
    bool JJWriteMemory(void* address,void *target, int type)
    {
        ...
        return writeMemory(address, target, len);
    }
    ...
    vector<void*> getResults(int count, int skip=0)
    {
        vector<void*> results;
        int index=0;
        for(int i=0; i<this->result->regions.size(); i++)
